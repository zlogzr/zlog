...
**ESM 的 import 是静态的**——构建工具在不运行代码的前提下，就能分析出"谁依赖谁、谁导出了什么没被用到"。这正是 tree-shaking 的前提（见 [[tree-shaking|Tree-shaking]]）。CJS 的 `require()` 可以写在 `if` 里、可以拼字符串，运行时才确定，静态分析无能为力。
